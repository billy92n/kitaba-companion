...
export function Sidebar({ campaign, entities, active, onNavigate, portraitUrl, mobileOpen = false, onCloseMobile }: Props) {
  ...
  return (
    <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`} aria-label="Navigation de campagne">
      ...
      <div className="sidebar-stat"><span>HP</span><strong>{hpCurrent !== null ? `${hpCurrent}${hpMax !== null ? ` / ${hpMax}` : ""}` : "—"}</strong></div>
      <div className="sidebar-stat"><span>Mana</span><strong>{manaCurrent !== null ? `${manaCurrent}${manaMax !== null ? ` / ${manaMax}` : ""}` : "—"}</strong></div>
      ...
      <button className={`gm-entry ${active === "gm_vault" ? "active" : ""}`} onClick={() => { onNavigate("gm_vault"); onCloseMobile?.(); }}>Coffre MJ</button>
    </aside>
  );
}
